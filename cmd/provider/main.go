...
import (
	...
	contextforgecontroller "github.com/entee28/provider-contextforge/internal/controller"

	"github.com/entee28/provider-contextforge/apis"
	"github.com/entee28/provider-contextforge/internal/version"
)
...
func main() {
	...
	log := logging.NewLogrLogger(zl.WithName("provider-contextforge"))
	if *debug {
	...
	kingpin.FatalIfError(apis.AddToScheme(scheme), "Cannot add ContextForge APIs to scheme")
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-contextforge",
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-contextforge:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(customresourcesgate.Setup(mgr, o), "Cannot setup CRD gate controller")
	kingpin.FatalIfError(contextforgecontroller.SetupGated(mgr, o), "Cannot setup ContextForge controllers")
